...
BEGIN
	...
	BEGIN
		...
		BEGIN
			PRINT 'IF (@intPerfil=1 OR @intPerfil=2 OR @intPerfil=3) y entramos a IF (@intDoctor = 0 AND @intProtesis = 0 AND @intProceso = 0)'
		...
		ALTER TABLE #tmpResultado ADD intProcesado INT
		--ALTER TABLE #tmpResultado ADD intAutorizado INT

		--UPDATE #tmpResultado SET intAutorizado = 2

		--SELECT '#tmpResultado',* FROM #tmpResultado

		UPDATE RES
		SET RES.intProcesado = PRO.isProcesadoEnd
		FROM #tmpResultado AS RES WITH(NOLOCK)
			JOIN tbJobHead AS PRO WITH(NOLOCK) ON PRO.OrderHead = RES.intOrdenLaboratorioEnc
		WHERE PRO.isProcesadoEnd = 1

		
		UPDATE RES
		SET RES.intAutorizado = (CASE 
									WHEN PRO.isAutorizadoEnd = 0 AND PRO.isYesosEnd = 1 THEN 0
									WHEN PRO.isAutorizadoEnd = 1 AND PRO.isYesosEnd = 1 THEN 1
									WHEN PRO.isAutorizadoEnd = 0 AND PRO.isYesosEnd = 0 THEN 2
								ELSE 2 END)
		FROM #tmpResultado AS RES WITH(NOLOCK)
			JOIN tbJobHead AS PRO WITH(NOLOCK) ON PRO.OrderHead = RES.intOrdenLaboratorioEnc
		--WHERE PRO.isAutorizadoEnd = 1
		
		--SELECT 'YA UODATE',* FROM #tmpResultado
		...
		SELECT R.intEmpresa, R.intSucursal, 
			...
			,intProcesado = ISNULL(R.intProcesado, 0)
			,intAutorizado = ISNULL(R.intAutorizado, 0)
...
qryOrdenLaboratorioEnc_SelDetalle_PorUsuario @strUsuario = 'MR-JOC',@intEsVersion2024=1,@intDoctor=0,@intProtesis=0,@intProceso=0 ,@intOrden=6616,@strPaciente=''
go
qryOrdenLaboratorioEnc_SelDetalle_PorUsuario @strUsuario = 'MR-JOC',@intEsVersion2024=1,@intDoctor=0,@intProtesis=0,@intProceso=0 ,@intOrden=6658,@strPaciente=''
go
qryOrdenLaboratorioEnc_SelDetalle_PorUsuario @strUsuario = 'MR-JOC',@intEsVersion2024=1,@intDoctor=0,@intProtesis=0,@intProceso=0 ,@intOrden=6745,@strPaciente=''
go
