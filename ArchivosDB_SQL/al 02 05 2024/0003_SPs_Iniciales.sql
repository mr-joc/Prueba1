...
go
CREATE PROCEDURE [dbo].[qry_infoUsuario_SEL]
...
BEGIN
	...
	FROM segUsuarios2024 AS S WITH(NOLOCK)
		JOIN tbRoles2024 AS R WITH(NOLOCK) ON S.introl = R.intRol
...
BEGIN
	...
	IF EXISTS(SELECT * FROM segUsuarios2024 WHERE strUsuario = @strUsuario AND strPassword = @strPassword AND isActivo = 1)
	BEGIN
		...
		FROM segUsuarios2024 WITH(NOLOCK)
...
go
--sp_GetMenuByRol 1
CREATE PROCEDURE [dbo].[sp_V2_GetMenuByRol]
	@IDRol int,
...
BEGIN
	...
	FROM tbMenu2024			AS M WITH(NOLOCK)
		JOIN tbMenuDtl2024	AS D WITH(NOLOCK) ON D.intMenu = M.intMenu
	...
	FROM #MenuSistemasBase	AS SIST WITH(NOLOCK)
		JOIN tbMenuXUsuario2024	AS  ME1 WITH(NOLOCK) ON ME1.IDMenu = SIST.IDMenu
	...
	FROM #MenuRol		AS MR1 WITH(NOLOCK)
		JOIN tbMenuXUsuario2024		AS DEL WITH(NOLOCK) ON DEL.IDMenu = MR1.IDMenu
