...
go
--qry_Color_Sel 0
CREATE PROCEDURE qry_V2_Color_Sel
@intColorID INT
AS
BEGIN
...
CREATE PROCEDURE qry_V2_AdeudosDoctor_PorUsuario
@intDoctor INT,
...
CREATE PROCEDURE qryTipoTrabajoPagado_Sel
(@intEmpresa INT, 
@intSucursal INT, 
@intPagado INT)
AS
BEGIN
	SET NOCOUNT ON;
	SELECT intTipo = 1, strNombre = 'SI'
	INTO #tmpResult
	UNION
	SELECT intTipo = 0, strNombre = 'NO'
	UNION
	SELECT intTipo = 2, strNombre = 'TODOS'

	SELECT intTipo, strNombre 
	FROM #tmpResult
	WHERE (intTipo = @intPagado OR @intPagado = 2)
END
go
...
CREATE PROCEDURE qry_V2_TipoTrabajo_Sel
